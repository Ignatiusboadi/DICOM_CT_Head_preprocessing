...
def skull_strip(nii_file, output_file=None, intensity=0.01):
    """
    Perform skull stripping on a NIfTI file using FSL's bet2 command.

    Parameters:
    -----------
    nii_file : str
        Path to the input NIfTI file (.nii or .nii.gz) to be skull stripped.
    output_file : str or None, optional (default=None)
        Path to save the skull stripped output file. If None, the output
        filename will be generated by appending '_brain' before the extension.
    intensity : float, optional (default=0.01)
        Fractional intensity threshold for bet2. Smaller values give larger brain outline estimates.

    Returns:
    --------
    str
        The path to the skull stripped output file.

    Notes:
    ------
    This function requires FSL to be installed and 'bet2' command accessible
    from the system's command line.
    """
    if output_file is None:
        output_file = nii_file.replace('.nii', '_brain.nii').replace('.nii.gz', '_brain.nii.gz')

    cmd = f"bet2 {nii_file} {output_file} -f {intensity} -v"
